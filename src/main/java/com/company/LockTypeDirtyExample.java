 ...
 * with {@link org.hibernate.annotations.OptimisticLockType#DIRTY} lock type.
 *
 * @author <a href="mailto:evfgesha@gmail.com">Eugenia Novikova</a>
 */
public class LockTypeDirtyExample {


  private static final int PERSON_COUNT = 2;

  private static CrudUtil<PersonLoyalEntity> crudUtil = new CrudUtil<>(PersonLoyalEntity.class);
  ...
  public static void main(String[] args) throws JsonProcessingException, InterruptedException {
    ...
    while (i < PERSON_COUNT) {
      crudUtil.insertPerson(PersonLoyalEntity.builder().id(UUID.randomUUID().toString()).name("InitialName_" + ++i).country("InitialCounty").build());
    }
    ...
    List<Callable<String>> callableList = Arrays.asList(
        ...
        () -> crudUtil.editPersonName(2000, personEntity.getId()),
        () -> crudUtil.editPersonName(2500, personEntity.getId())
    );
