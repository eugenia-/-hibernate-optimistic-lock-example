...
public class VersionedExample {
  ...
  private static CrudUtil<PersonVersionedEntity> crudUtil = new CrudUtil<>(PersonVersionedEntity.class);
  ...
  public static void main(String[] args) throws JsonProcessingException, InterruptedException {
    ...
    while (i < PERSON_COUNT) {
      crudUtil.insertPerson(PersonVersionedEntity.builder().id(UUID.randomUUID().toString()).name("InitialName_" + ++i).country("InitialCounty").build());
    }
    ...
    IPersonEntity personEntity1 = crudUtil.listPersons().get(0);
    IPersonEntity personEntity2 = crudUtil.listPersons().get(1);
